       ...
       Description("Currency Symbol"),
        ...
        private HyperLink GetHyperLink(string ColumnName, string Value, DataRow DataSource)
        {
            ...
            if (_HyperLinks != null)
            {
                ...
                {
                    if (ColumnName.ToLower() == ((TradingBell.WebServices.UI.HyperlinkColumn)_HyperLinks[iCtr]).FieldName.ToLower())
                    {
                        retHyp = new HyperLink();
                        retHyp.NavigateUrl = TBParse(((TradingBell.WebServices.UI.HyperlinkColumn)_HyperLinks[iCtr]).URL.ToString(), DataSource);
                        retHyp.Text = Value;
                    }
                }
            }
