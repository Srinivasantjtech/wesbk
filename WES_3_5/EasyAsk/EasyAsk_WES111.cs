                            ...
                            using (DataTable td = YHSAndBC.Tables["YHSAndBC"])
                            {
                                ...
                                else if (td.Rows[i]["TempPath"].ToString().ToLower().Contains("search"))
                                {
                                   ...
                                   //if (td.Rows[i]["ItemType"].ToString().ToLower() == "product")
                                    //{
                                    if (td.Rows[i]["ItemType"].ToString().ToLower() == "product" && objhelper.CDEC(td.Rows[i]["Itemvalue"]) > 0)
                                    {
                        ...
                        for (int i = 0; i <= YHSAndBC.Tables["YHSAndBC"].Rows.Count - 1; i++)
                        {
                            using (DataTable td = YHSAndBC.Tables["YHSAndBC"])
                            {
                                ...
                                if (td.Rows[i]["TempPath"].ToString() == "/Category")
                                {
                                    ...
                                    Ds =(DataSet)objhelperDb.GetGenericDataDB(td.Rows[i]["ItemValue"].ToString(), "GET_CATEGORY_ID_PARENT", HelperDB.ReturnType.RTDataSet);
                                ...
                                else if (td.Rows[i]["TempPath"].ToString() == "/CableL")
                                {
                                ...
                                else if (td.Rows[i]["TempPath"].ToString() == "/Brand")
                                {
                                ...
                                else if (td.Rows[i]["TempPath"].ToString() == "/Brand/Model")
                                {
                                ...
                                else if (td.Rows[i]["TempPath"].ToString().StartsWith("/Category/Category") || td.Rows[i]["TempPath"].ToString().StartsWith("/Category/Family") || td.Rows[i]["TempPath"].ToString().StartsWith("/CableL") || td.Rows[i]["TempPath"].ToString().StartsWith("/Brand"))
                                {
                                    if (isProduct == true)
                                    {
                                        //if (td.Rows[i]["ItemType"].ToString().ToLower() == "product")
      ...
      //Added by indu
                            ...
                            for (int i = 2; i <= ds.Tables[0].Rows.Count - 1; i++)
                            {
                                if (tempds != null && tempds.Tables.Count > 0)
                                {
                                    ...
                                    for (int i1 = 0; i1 <= tempds.Tables.Count - 1; i1++)
                                    {
                                        ...
                                        if (tempds.Tables[i1].TableName.ToLower().Contains(row["ItemType"].ToString().ToLower()) && checkAddedtable.Contains("," + i1.ToString() + ",") == false)
                                        {
                                            inxtbl = i1;
                                            break;
                                        }
        ...
        public DataSet GetCategoryAndBrand(string ReturnType)
        {
            if (HttpContext.Current.Session["MainCategory"] == null || HttpContext.Current.Session["SubCategory"] == null ||  HttpContext.Current.Session["Filter"] == null )
            {
                ...
                System.IO.FileInfo Fil2 = new System.IO.FileInfo(strxml + "\\Filters.xml");
        ...
        private DataTable Get_Family_Product_ValuesJson(string DataType, DataTable ds, INavigateResults res, IResultRow item1, String name, DataSet Dssdv)
        {
            ...
            string _FmlylongDesc = "";
            string image_string = "";
            string tobedeletepids = "";
            int sno = 0;

            string temp_family_count = "";

            string temp_product_count = "";
                            ...
                            foreach (DataRow dr in Sqltb.Rows)
                            {
                                foreach (DataRow dr1 in ds.Rows)
                                {
                                    if (dr["FAMILY_ID"].ToString().ToUpper() == dr1["FAMILY_ID"].ToString().ToUpper())
                                    {
                                        ...
                                        dr1["CATEGORY_PATH"] = dr["CATEGORY_PATH"].ToString();    
                                        dr1["CUSTOM_NUM_FIELD3"] = ((int)float.Parse(dr["CUSTOM_NUM_FIELD3"].ToString())).ToString();
                                    }
                                }
                            }
